...
laps$sec_round <- laps$milliseconds / 1000 
# Create a variable for seconds, for easier understanding and interpretability.
...
head(laps)
 ...
 laps %>%
  ...
  ggplot(aes(sec_round, fill=position, colour = position)) +
  geom_density(alpha = 0.1) + xlim(50,200) + theme_economist()
...
avg_lap <- laps %>%
  ...
  summarise(mean_race = mean(sec_round, na.rm = T))  
# Calculate the mean lap time per race for a mean lap


races$raceId <- as.character(races$raceId)
f1 <- left_join(races, avg_lap,by = c("raceId")) %>%
  select("raceId", "name", "mean_race", "year" )
...
ggplot( aes(mean_fastest )) + geom_density(alpha=.2, fill = "darkgreen") +
  geom_density(aes(x=mean_race), alpha=.2, fill= "red")  + theme_economist() + 
  scale_fill_brewer(palette = "Set3")  + 
  theme(legend.position="bottom") +
  labs(title = "Distribution of Laptimes for the Monaco GP 2005 - 2017", subtitle = 
         "Red: Average lap times for average rounds, Green: Fastest laps",
       y="Density", x= "Lap Time") 
...
x <- ggplot(f1, aes(x = abs(year),
  ...
  theme_economist() + 
  labs(title = "Laptimes in F1 over time", subtitle = "Grouped by Grand Prix, 
       the line represents the fastest laps, the dots represent average laptimes.",
